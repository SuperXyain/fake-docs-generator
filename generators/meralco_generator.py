...
class MeralcoGenerator(DocumentGenerator):
    """Generate realistic MERALCO electricity bills."""

    def generate_consumption_graph(self, current_consumption, filename='temp_graph.png'):
        ...
        current_year_data.append(current_consumption)
        ...
        # Create bars
        ax.bar([i - width/2 for i in x], previous_year_data, width, label='2023', color='#CCCCCC')
        ax.bar([i + width/2 for i in x], current_year_data, width, label='2024', color='#333333')
    ...
    def generate_qr_code(self, can, amount, filename='temp_qr.png'):
        """Generate QR code for payment."""
        qr_data = f"MERALCO|CAN:{can}|AMOUNT:{amount:.2f}"
    ...
    def generate_bill(self, filename, bill_num=1):
        ...
        # Generate CAN (Customer Account Number)
        ...
        # Dates
        start_date, end_date, due_date = self.get_random_date_range(30)
        ...
        # Try to add logo
        logo_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'assets', 'meralco_logo.png')
        try:
...
Route Seq.: {route_seq}    Print Seq.: {print_seq}"""
        ...
        # Bill title
        billtitle_style = ParagraphStyle('billtitle', parent=styles['Normal'], fontSize=11,
        ...
        bill_info_data = [[Paragraph(bill_period_text, info_style), Paragraph(bill_date_text, info_style)]]
...
<b>Current Reading</b><br/>{current_reading:,}"""
        ...
        reading_info_data = [[Paragraph(reading_date_text, info_style), Paragraph(meter_info_text, info_style)]]
        ...
        consumption_header_data = [[Paragraph(previous_text, consumption_header), "",
                                    Paragraph(actual_text, consumption_header)]]
        ...
        amount_due_style = ParagraphStyle('amount_due', parent=normal_text, fontSize=20,
        ...
        graph_file = None
        try:
            graph_file = f'outputs/temp_graph_{bill_num}.png'
            self.generate_consumption_graph(consumption, graph_file)
...
vs same period last year"""
        ...
        if remaining_balance > 0:
            ...
            charges_data_list.append([
                Paragraph('<i>(see details under What Remains Unpaid; does not include bills under review and installment)</i>',
                          charges_style),
                ''
            ])
        ...
        charges_data_list.extend([
            ...
            [Paragraph('<b>Total Amount Due</b>',
                       ParagraphStyle('total', parent=charges_style, fontName='Helvetica-Bold')),
             Paragraph(f'<b>₱ {total_current:,.2f}</b>',
                       ParagraphStyle('total_amt', parent=charges_style, fontName='Helvetica-Bold'))]
        ])
